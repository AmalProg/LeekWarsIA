...
def getAiCode(ai) :
    time.sleep(0.4)
    resp = requests.get(lwAPIUrl + "ai/get/" + str(ai['id']), cookies=cookies)
...
"""
def exportAI() :
    loginData = {'login': 'Amal', 'password': 'Miku35310*'}
    loginResp = requests.post(lwAPIUrl + 'farmer/login-token/', loginData)
    if loginResp.status_code == 200 :
        global cookies
        cookies = loginResp.cookies

        aisResp = requests.get(lwAPIUrl + 'ai/get-farmer-ais', cookies=cookies)
